...
export const languagesAsItems = fallbackLanguagesList.map((language) => ({
  value: language.id, 
  label: language?.aliases?.length ? language.aliases[0] : language.id,
}));

export const getLanguageAlias = (languageId: string) => {
    const language = fallbackLanguagesList.find((language) => language.id === languageId);
    return language?.aliases?.length ? language.aliases[0] : languageId;
}
