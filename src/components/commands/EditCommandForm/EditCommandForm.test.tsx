...
test("Displays default values correctly", async () => {
  const { getByText } = customRender(<EditCommandForm commandItem={testCommand} onClose={vi.fn()} />);
...
test("Displays validation messages correctly", async () => {
  const { findByText, getByRole, getByPlaceholderText } = customRender(<EditCommandForm commandItem={testCommand} onClose={vi.fn()} />);
...
test("Submits form correctly", async () => {
  const { getByRole, getByPlaceholderText } = customRender(<EditCommandForm commandItem={testCommand} onClose={vi.fn()} />);
  const submitButton = getByRole("button", { name: /Save command/i });
